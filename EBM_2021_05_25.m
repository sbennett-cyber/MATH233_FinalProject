...
EBM(CenterID, Center, Cross, R)

function [out] = EBM(CenterID, Center, Cross, R)
            
            find(obj.Network.Boundary(1)==CenterID) %Find the domain in which the crosspoint lives
            %Search for boundries shared by the center domain and
            %crosspoint
            v1=[0,8];
            v2=[3.5,3.5];
            [d, Intercept, Type, m_and_b] = point_to_line(pt, v1, v2,"Plot") %Find the normal passing through the crosspoint and normal to a boundary line
            1
