classdef NetworkLine2
    ...
    methods 
        ...
        function obj = NetworkLine2(poly_, net_, BoundaryX, BoundaryY, PntsField, PntsLine, alpha_, beta_)
                ...
                switch Case
                   case "ZeroDirichlet"
                      ...
                      LocVertex2 = obj.NetMtrix(2).LocVertex;
                       ...
                       Coeff = 1/((obj.NetMtrix(1).D + obj.NetMtrix(2).D)*(3/2)); %Find the correct coefficient
                       
                       if (LocVertex2==1 & LocVertex1==Size(1)) %If channel 2 is connected on the right to channel 1

                           Fix(Size(1)-2) =  Coeff*2*obj.NetMtrix(1).D; %Create the row of coefficients needed to edit
                           Fix(Size(1)-3) = -Coeff*(1/2)*obj.NetMtrix(1).D;
                           Fix(Size(1)-1) =  Coeff*2*obj.NetMtrix(2).D;
                           Fix(Size(1))   = -Coeff*(1/2)*obj.NetMtrix(2).D;
                           
                           FixLoc = [Size(1)-2;Size(1)-1];
                           
                       elseif (LocVertex1==1 & LocVertex2==Size(1)) %If channel 2 is connected on the right to channel 1
                           
                           Fix(1) =  Coeff*2*obj.NetMtrix(1).D; %Create the row of coefficients needed to edit
                           Fix(2) = -Coeff*(1/2)*obj.NetMtrix(1).D;
                           Fix(end) =  Coeff*2*obj.NetMtrix(2).D;
                           Fix(end-1)   = -Coeff*(1/2)*obj.NetMtrix(2).D;
                       ...
                       LHS(FixLoc,:)  = LHS(FixLoc,:) +[obj.NetMtrix(1).r*Fix;obj.NetMtrix(2).r*Fix]; %Update matrix with fix times the correct r for the domain
                       RLHS(FixLoc,:) = RLHS(FixLoc,:)+[obj.NetMtrix(1).r*Fix;obj.NetMtrix(2).r*Fix];
...
%                                 hold on 
        ...
        function [out1, out2, out3, out4, out5] = EBM(obj,poly, CenterID, Center, Cross)
            ...
            if (XSize(1) > 3) 
                ...
                if LocMax >= (length(X_Cross)-1)/2
                    X_Cross = X_Cross(1:LocMax-1,:);
                elseif LocMax < (length(X_Cross)-1)/2
                    X_Cross = X_Cross(LocMax+1:end,:);
                else
                    "XSize Incorrect"
                pause
                end
            ...
            if (YSize(1) > 3) 
                ...
                LocMax = find(abs(Diff-max(Diff))<obj.tol);
                LocMin = find(abs(Diff-min(Diff))<obj.tol);
                if LocMax >= (length(Y_Cross)-1)/2 & abs(LocMax-LocMin)>obj.tol
                    Y_Cross = Y_Cross(1:LocMax-1,:);
                elseif LocMax < (length(Y_Cross)-1)/2 & abs(LocMax-LocMin)>obj.tol %#ok<*AND2>
                    Y_Cross = Y_Cross(LocMax+1:end,:);
                else
...
%                             Coeff_CGamma    = obj.LagrIP_CGamma(YSize(1)+1, xi_bar, xi_bar, ka, kd, poly(CenterID).D);                    % Calculate Lagrange IP coefficents for C_Gamma
%                             CoeffRho_CGamma = Coeff_CGamma(end);                                              % Identify the coefficent on the rho term
% 
%                             Coeff_CBar = obj.LagrIP_CBar(YSize(1)+1, xi_bar, xi_bar, xi_bar, Coeff_CGamma); % Calculate Lagrange IP coefficents for C_bar
%                             CoeffRho_CBar = Coeff_CBar(end); %Identify coefficient on rho term
%                             InterpCBar = [Y_Cross,Coeff_CBar(1:end-1)'];
%                             Coeff_Rho = Coeff_CBar(end);
%                             out4 = Y_Cross;
