...
% load('KL00100100kd1V1_Net.mat')
% load('KL00100100kd1V1_poly.mat')
...
PntsInX = 81;
PntsInY = 81;
PntsLine = [161, 0, 0]; %This is a vector when the network has more than one channel
...
n = 20;
Time = (n/dt);
C = NetworkLine2(poly,net, [0,L],[0,L],[PntsInX,PntsInY], PntsLine, alpha, beta); %Initialize the class with polygons (vertec of each subdomain & the diffusion and growth terms) and net from .mat files, min and max in X, min and max in Y, [Pnts in X,Pnts in y,dt]
...
for i = 1:length(PntsInX)
    C = NetworkLine2(poly,net, [0,16],[0,16],[PntsInX(i),PntsInX(i)], PntsLine(i,:), alpha, beta); %Initialize the class with polygons (vertec of each subdomain & the diffusion and growth terms) and net from .mat files, min and max in X, min and max in Y, [Pnts in X,Pnts in y,dt]
...
PntsLine = [201,0,0; 401,0,0;801,0,0; 1601,0,0; 1,0,0];
drho = L./(PntsLine(:,1)-1);
dt = L./(PntsInX(:,1)-1);
dx = L./(PntsInX(:,1)-1);
